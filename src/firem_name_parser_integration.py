...
def match_descriptor(descriptor: str, rules: dict) -> Tuple[Optional[str], Dict[str, Any]]:
...
def _extract_material_name(node: Any) -> Optional[str]:
    if not isinstance(node, dict):
        return None

    candidate_keys = [
        "material_name", "MaterialName", "material", "Material",
    ...
    for k, v in node.items():
        if isinstance(k, str) and "material" in k.lower() and isinstance(v, str) and v.strip():
            return v.strip()

    return None


def _extract_aggregate_state(node: Any) -> Optional[str]:
    if not isinstance(node, dict):
        return None
...
def build_phase_map_from_asix(asix_dict: dict) -> Dict[str, Dict[str, Dict[str, Any]]]:
    ...
    named_nodes = {path: _extract_name(node) for path, node in all_nodes if _extract_name(node)}
    ...
    for path, node in all_nodes:
        ...
        for i in range(len(path), -1, -1):
            ...
            if candidate_name and not re.fullmatch(r"Phase\s+\d+", candidate_name.strip()):
...
def enrich_with_asix(
    parsed_record: dict,
    asix_dict: Optional[dict] = None,
    phase_map: Optional[Dict[str, Dict[str, Dict[str, Any]]]] = None,
    domain_lookup: Optional[Dict[str, Dict[str, Any]]] = None,
) -> dict:
    ...
    if phase_label:
        for domain_key in [inner_domain, top_domain]:
            if domain_key and domain_key in phase_map and phase_label in phase_map[domain_key]:
                ...
                if phase_info.get("phase_material_name"):
                    qualifiers["phase_material_name"] = phase_info["phase_material_name"]
                qualifiers["phase_resolved_from"] = domain_key
                break

    # Domain enrichment
    for domain_key, field_prefix in [(top_domain, "top_domain"), (inner_domain, "internal_domain")]:
...
def normalize_2d_results_columns(
    ...
    domain_lookup = build_domain_lookup_from_asix(asix_dict) if asix_dict is not None else None

    for col in df.columns:
        parsed = parse_firem_header(str(col), rules)
        enriched = enrich_with_asix(parsed, asix_dict=asix_dict, phase_map=phase_map, domain_lookup=domain_lookup)
        ...
        rows.append(
            {
                ...
                "qualifiers_json": json.dumps(enriched["qualifiers"], ensure_ascii=False, sort_keys=True),
